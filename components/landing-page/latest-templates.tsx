...
export default function LatestTemplates() {
  ...
  return (
    <section className="py-16 p-6 relative mb-16">
      <div className="max-w-screen-xl w-full mx-auto text-center flex flex-col">
        ...
        >
          <CarouselContent className="items-center w-full h-full">
            {Templates.map((template, index) => (
              <CarouselItem
                className={`lg:basis-1/3 transition-transform duration-300 ${
                  index === selectedIndex + 1
                    ? "lg:basis-1/2 scale-80 scale-90"
                    : "scale-85"
                }`}
                key={template.id}
              >
